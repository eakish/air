...
%     [callSeq.crossCorr.soundVSair{i}, callSeq.crossCorr.lags{i}] = xcorr(envSong ./ max(envSong), data{i}.air ./ max(data{i}.air));
    ...
    for j = 1 : length(inspEnd)
        if (expPeak(j) > callThresh) && (expDur(j) > 100 * fs / 1000)
            callExpDur = length([find(air(breathStartInd(j) : breathEndInd(j)) > 2000)]);
            if callExpDur < 200 * fs / 1000
                c = [c inspEnd(j)];
                % === make sure that the sound crosses an amplitude threshold ===
            end
        end
    end
    callTimes = c;
...
%         forHamish.callOnset = callTimes;
%         forHamish.breathing = data{i}.air;
%     end
...
%     expOn_preCall4 = inspEnd(expOn_preCall(end - 3));
    ...
    inspDur_preCall1 = [inspDur_preCall1 inspDur(inspOn_preCall(end)) * 1000 / fs];
    expDur_preCall1 = [expDur_preCall1 expDur(expOn_preCall(end - 1)) * 1000 / fs]; % the last expiration is the call, end - 1 is the expiration preceding the call
...
cd('preProcessed2')
save(strcat(bird, '_', date, '_airCalls')); % insert variables to save
